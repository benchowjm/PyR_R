...
If you run R on your own personal computer, the easiest way to install packages is to type a single line of code into the console. This accesses an online 'repository' containing the files and downloads them to your computer. For instance to install the package *shiny* you would type:
```{r, eval=F}
...
1. Access the package website on the CRAN network. For example, for the package 'htmltools' click [here](https://cran.r-project.org/web/packages/htmltools/index.html)
    + You can replace *htmltools* in the address with the package of your choice
...
2. Check if your package has any 'dependencies'
    ...
    + For the package [htmltools](https://cran.r-project.org/web/packages/htmltools/index.html) there are 2 dependencies: *utils* (pre-installed) and *digest*
...
### 5. Short Familiarisation Exercise ###
...
```{r, eval=F}
animals <- data.frame(type=c("Cat", "Dog", "Elephant"), name=c("Cathy", "Donald", "Ervin"))
```
